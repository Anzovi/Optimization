...
Original file is located at
    https://colab.research.google.com/drive/12PYkN6psmak8Mz19951a3MpitPC3C9Sz
"""
...
class QuadraticOracle(BaseSmoothOracle):
    ...
    def minimize_directional(self, x, d):
        ...
        return -np.dot(self.grad(x).T, d) / np.dot(d.T, self.A.dot(d))
...
class LogRegL2Oracle(BaseSmoothOracle):
    ...
    def hess_vec(self, x, d):
        ...
        return self.matvec_ATx(sigma * (1.0 - sigma) * self.matvec_Ax(d))/self.b.size + self.regcoef * d
...
def hess_vec_finite_diff(func, x, v, eps=1e-5):
    ...
    basis_ort = np.eye(x.size)
    func_for_each_e_i = lambda e_i: func(x + eps * v + eps * e_i) - func(x + eps * e_i)
    basis_ort_eps = np.apply_along_axis(func_for_each_e_i, 1, basis_ort)

    return (basis_ort_eps - func(x + eps * v) + func(x)) / eps**2